...
from .forms import PartsMasterUpdateForm, IssueForm, ReceiveForm, IssueCreateForm
...
def update_items(request, pk):
	...
	form = PartsUpdateForm(instance=queryset)
	if request.method == 'POST':
		form = PartsUpdateForm(request.POST, instance=queryset)
		if form.is_valid():
			form.save()
			messages.success(request, 'Successfully Saved')
...
def delete_items(request, pk):
	queryset = Parts.objects.get(id=pk)
	if request.method == 'POST':
		queryset.delete()
